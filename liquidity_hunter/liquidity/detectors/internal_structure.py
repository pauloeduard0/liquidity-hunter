...
class InternalStructureDetector(MarketStructureDetector):
    ...
    *pending* from the flip that set it (a CHoCH, a `CHOCH_FAILED` revert, or
    ...
    `choch_pending_fail_at_broken_level` (default `False`) is the pending
    half of the same hysteresis: a *pending* CHoCH (no emitted BOS confirmed
    it yet) also arms **its own broken level** as an invalidation reference,
    even when that reference was structural -- so a sustained close back
    through the level fails the CHoCH (`CHOCH_FAILED`, real trend flip)
    without waiting for the far leg-origin reclaim. Without it, an impulsive
    counter-move that never printed a confirming BOS leaves both exits
    (CHOCH_FAILED at the origin, reverse CHoCH at `choch_origin_<side>`)
    pinned at the reversed leg's extreme, and a full recovery rally prints as
    a chain of sweeps under a stale trend (the AAVEUSDT H1 2026-07-08 case:
    a bearish CHoCH at the structural 87.90, no bearish BOS, then +14% of
    rally read as three bullish sweeps until the 97.4 origin broke three days
    later). The failure at the structural level demands
    `choch_pending_fail_persistence_candles` sustained closes (default `None`
    = base persistence) -- typically set above base so an ordinary retest of
    the broken level does not kill a genuine reversal; weak references keep
    the existing `choch_weak_ref_fail_at_broken_level` behavior (base
    persistence) and the confirming BOS retires the level like it retires the
    origin. A structural-level failure restores the pre-CHoCH staircase stash
    (the interrupted cycle was alive), unlike a weak-level failure's re-seed
    at the reclaimed level. With `False` the output is byte-for-byte
    identical.

    `choch_fail_level_buffer_atr` (default `None` = off) puts a
    volatility-scaled **noise band** around every level-armed CHoCH-failure
    reclaim. All the failure checks measure the reclaim against a bare price,
    and price retests a broken level constantly, so two closes a hair past one
    invalidated the reversal at base persistence: the BTCUSDT M15 bullish CHoCH
    of 2026-07-25 18:00 (ref 64305.8) held ten candles, dipped a maximum of
    0.37 mean-TR below the level for four, was killed -- and re-fired at the
    same level an hour later into a +1.9% leg, printing `CHoCH ✕ ▲` and
    `CHoCH ↻ ▲` on one price. When set, the reclaim must *clear* the level by
    N x the series' mean true-range% before it counts, so a routine pullback
    into the counter-zone no longer negates a CHoCH while a genuine loss of the
    level still does (the failure simply confirms a few candles later, at the
    close that actually clears the band). Applies only where the fail pivot is
    the CHoCH's armed *level* (`choch_weak_ref_fail_at_broken_level` /
    `choch_pending_fail_at_broken_level`, live-edge re-run included): a reclaim
    of the leg **origin** is already a deep, unambiguous invalidation and keeps
    ...
    **post-failure fallback suppression**. A failed-CHoCH flip arms no
    ...
    `choch_failed_rearm` (default `False`) is the **failed-CHoCH
    re-activation**. A `CHOCH_FAILED` permanently discards its CHoCH: the flip
    arms no origin (one-shot, anti-ping-pong) and the fallback is suppressed
    (above), so when the "reclaim" that failed the CHoCH turns out to be the
    last gasp of the old trend -- price rolls back over and sustains beyond
    the very level the CHoCH had broken -- nothing re-fires, and the resumed
    move prints as a chain of sweeps under the wrong trend (the MUUSDT H4
    2026-07 case: a bearish CHoCH at 1026.86 failed on a flat drift hugging
    the level, then the -19% collapse below it read as two sweeps while the
    trend sat bullish). When set, the failure arms the failed CHoCH's broken
    reference as a *re-arm* level: a later sustained break back beyond it
    (scanned only from the failure onward, at the original reference's
    weak/structural persistence class) re-emits the `CHANGE_OF_CHARACTER` and
    flips the trend again. The re-arm ranks just above the trailing fallback
    in the reference chain (structural references keep their authority) and
    is exempt from the post-failure fallback suppression (it is a proven
    structural fact, not a hair-trigger trailing pivot). One-shot per failure
    chain: a re-fired CHoCH's own
    failure does not re-arm (no ping-pong); the memory is dropped when any
    CHoCH emits (a fresh cycle owns the narrative) or when the opposite trend
    is *confirmed* (emitted BOS or displacement-success -- the reclaim built
    real structure, so a much-later break of the old level is a fresh
    reversal, not a re-activation). With `False` the output is byte-for-byte
    identical.

    `choch_failed_rearm_persistent` (default `False`) makes the re-arm
    memory *persistent* while the level stays contested: (a) opposite-trend
    confirmation (an emitted opposite BOS or displacement-success) *demotes*
    the re-arm to the very end of the reference chain -- below the trailing
    fallback -- instead of retiring it, and (b) a failed *re-fire* arms the
    level again (the one-shot-per-chain guard is lifted). A demoted re-arm
    coexisting with a live fallback is arbitrated by *whichever level price
    crosses first in the break direction* (bearish: the higher level;
    bullish: the lower). Both halves of that rule are load-bearing: at full
    rank a far armed level shadows every nearer live reference (on the
    BTCUSDT D1 fixture a bullish re-arm at 94760, armed by the January 2026
    failure, would sit above the whole March-May rally and silently swallow
    the April CHoCH that fires at the 75998.9 fallback), while a strict
    below-the-fallback rank loses the early re-fire whenever a farther
    trailing fallback exists (the October 2025 target re-fires at 111850,
    crossed days before the 103470 trailing low the fallback held).
    Rationale (the BTCUSDT D1 2025-08 case): a bearish CHoCH at 111850
    failed on a sustained reclaim; a late-September dip re-fired it, the
    re-fire failed on the October rally to a marginal new high (126208 over
    124546, one continuation BOS at the top), and the one-shot chain died --
    so when the whole leg was given back in November (the top was a
    liquidity sweep; the original CHoCH was right) nothing could re-fire,
    and the crash's reversal waited for a late, weak trailing reference
    (98889, eleven candles later). With the memory persistent, the sustained
    November break back through 111850 re-fires the CHoCH at the proven
    structural level. Ping-pong stays bounded: every flip still demands its
    own sustained persistence (failure paths included), only the *most
    recent* failure's level is remembered per side, a fresh CHoCH emission
    replaces the memory with its own cycle, and the composition pass that
    collapses failed re-fire cycles keeps the chart to the original failure
    plus the surviving re-fire. Requires `choch_failed_rearm`. With `False`
    the output is byte-for-byte identical.

    `choch_fail_live_edge` (default `False`) is the **live-edge failure
    emission**. The `CHOCH_FAILED` checks are pivot-gated -- the failure is
    emitted while processing the next swing pivot -- so on a relentless
    one-way move (every candle a new extreme, no pivot forming for days) a
    CHoCH whose fail level has long been sustained-broken keeps its wrong
    trend standing at the live edge (the MUUSDT H4 2026-07-14 re-fired
    bullish CHoCH: price fell 19% below the 962.15 level, six closes past the
    pending-fail persistence, and the trend sat bullish because the vertical
    drop never formed a low pivot; the additive fizzle marker showed but
    never flips the trend). The failure condition is complete -- only the
    messenger is late. When set, the same check (same reference arbitration,
    same persistence class, same displacement-success retirement) runs once
    more at the end of `detect` over the final state, and a confirmed
    sustained break emits the real `CHOCH_FAILED` (trend flip included) at
    the sustained-break candle without waiting for the pivot.
    Batch-deterministic: once the closes exist, every later run re-derives
    the same event, and when the pivot finally forms the in-loop path emits
    it identically (with the full failure bookkeeping, which only matters
    for subsequent pivots). The provisional/fizzle live-edge marks are
    suppressed on the run that emits a live-edge failure (their floor state
    reflects the pre-flip trend). With `False` the output is byte-for-byte
    identical.
    ...
    def __init__(
        ...
        if choch_fail_level_buffer_atr is not None and choch_fail_level_buffer_atr < 0:
            raise ValueError("choch_fail_level_buffer_atr must be non-negative")
    ...
    def detect(
        ...
        *,
        ...
        # `choch_pending_fail_at_broken_level` path, pending-fail persistence
        # + pre-CHoCH staircase restore). Only read while the matching
        # `*_choch_fail_ref` is armed.
        ...
        # Failed-CHoCH re-activation (`choch_failed_rearm`): the broken
        # reference of the most recently *failed* CHoCH of each direction. A
        # later sustained break back beyond it (scanned only from the failure
        # onward, via `*_choch_rearm_arm_index`) re-fires the CHoCH -- the
        # "reclaim" that failed it was the old trend's last gasp, not a real
        # recovery. `*_choch_rearm_weak` carries the original reference's
        # weak/structural class (persistence + re-failure arming);
        # `*_choch_from_rearm` marks the standing CHoCH as a re-fire, so its
        # own failure does not re-arm (one-shot per failure chain, no
        # ping-pong). Dropped at any CHoCH emission or when the opposite trend
        # is confirmed (emitted BOS / displacement-success).
        bear_choch_rearm: Pivot | None = None
        bear_choch_rearm_weak = False
        bear_choch_rearm_arm_index = -1
        bear_choch_from_rearm = False
        bull_choch_rearm: Pivot | None = None
        bull_choch_rearm_weak = False
        bull_choch_rearm_arm_index = -1
        bull_choch_from_rearm = False
        # Under `choch_failed_rearm_persistent`, opposite-trend confirmation
        # *demotes* the re-arm instead of retiring it: it drops to the very
        # end of the reference chain (below the trailing fallback), so a far
        # armed level cannot shadow nearer live references -- the reclaim
        # built real structure, so fresh references own the reversal read --
        # while the level still catches a full round trip when no other
        # reference exists (e.g. inside the post-failure fallback
        # suppression window).
        bear_choch_rearm_demoted = False
        bull_choch_rearm_demoted = False
        ...
        for timestamp, kind, price in pivots:
            ...
            if kind == "high":
                ...
                if pending_bos is not None and pending_bos.direction is MarketDirection.BEARISH:
                    ...
                    if (
                        ...
                        if self._pullback_quality_ok(candles[current_index], high_pivot=True):
                            ...
                            # bullish reversal CHoCH. A confirmed bearish trend
                            # also retires the bullish re-arm memory (see
                            # `choch_failed_rearm`): the reclaim that failed
                            # the old bullish CHoCH built real structure --
                            # or, under `choch_failed_rearm_persistent`, is
                            # demoted below the fallback to catch a
                            # sweep-shaped bottom without shadowing nearer
                            # references.
                            trend_confirmed = True
                            if self._choch_failed_rearm_persistent:
                                bull_choch_rearm_demoted = True
                            else:
                                bull_choch_rearm = None
                        elif (
                ...
                if (
                    ...
                    fallback_active_high = None
                # Failed-CHoCH re-activation reference (`choch_failed_rearm`):
                # ranks just above the trailing fallback -- structural
                # references (validated / pending leg origin / blind-spot
                # origin) keep their authority while armed -- and is exempt
                # from the post-failure fallback suppression above (the level
                # is a proven structural fact, not a hair-trigger trailing
                # pivot).
                rearm_high_ref = bull_choch_rearm if self._choch_failed_rearm else None
                choch_high_ref = (
                    validated_choch_high
                    or pending_leg_origin_high
                    or choch_origin_high
                )
                if choch_high_ref is None:
                    if (
                        bull_choch_rearm_demoted
                        and rearm_high_ref is not None
                        and fallback_active_high is not None
                    ):
                        # A demoted re-arm (`choch_failed_rearm_persistent`,
                        # after opposite-trend confirmation) no longer
                        # outranks a live fallback: whichever level price
                        # crosses *first* in the break direction governs (a
                        # bullish CHoCH breaks upward, so the lower level).
                        # A far-overhead armed level must not shadow a nearer
                        # live reference, but a re-arm *nearer* than the
                        # fallback still catches the round trip early.
                        choch_high_ref = (
                            rearm_high_ref
                            if rearm_high_ref.price <= fallback_active_high.price
                            else fallback_active_high
                        )
                    else:
                        choch_high_ref = rearm_high_ref or fallback_active_high
                ...
                # A re-arm reference keeps its original CHoCH's classification
                # (see `choch_failed_rearm`).
                if rearm_high_ref is not None and choch_high_ref is rearm_high_ref:
                    choch_high_weak_ref = bull_choch_rearm_weak
                ...
                # that armed it (mirror of the `*_choch_arm_index` clamp on the
                ...
                if (
                    ...
                    # confirming BOS -- see `choch_failed_rearm` /
                    # `choch_failed_rearm_persistent`).
                    trend_confirmed = True
                    if self._choch_failed_rearm_persistent:
                        bull_choch_rearm_demoted = True
                ...
                # A failure at a *structural* broken level (the pending-fail
                # path) demands its own persistence: an ordinary retest of a
                # genuine leg origin must not kill the reversal at base
                ...
                if (
                    ...
                    and not bear_choch_fail_ref_weak
                ...
                if (
                    ...
                    # CHoCH drop launched from (or, for a level-armed CHoCH,
                    ...
                    bull_leg_high = price
                    # Weak-level failures re-seed at the reclaimed level (the
                    # old cycle was spent); a *structural*-level failure (the
                    # pending-fail path) restores the pre-CHoCH stash below --
                    # the interrupted cycle was alive and its staircase is
                    # genuine.
                    weak_level_failure = (
                        bear_fail_pivot is not bear_choch_origin
                    ...
                    pre_choch_bull_bos_extreme = None
                    # Failed-CHoCH re-activation (`choch_failed_rearm`): arm
                    # the failed bearish CHoCH's broken level so a later
                    # sustained break back below it re-fires the CHoCH --
                    # unless this CHoCH was itself a re-fire (one-shot per
                    # failure chain, no ping-pong; lifted under
                    # `choch_failed_rearm_persistent`, which keeps the level
                    # armed while it stays contested). The re-arm pivot keeps
                    # the level's *price* but carries the *failure's*
                    # timestamp, so the re-fired CHoCH's line (anchored at its
                    # reference_timestamp) starts where the failure ended
                    # rather than re-drawing the original CHoCH's whole span
                    # on top of the failure line.
                    if (
                        self._choch_failed_rearm
                        and (
                            not bear_choch_from_rearm
                            or self._choch_failed_rearm_persistent
                        )
                        and standing_choch_dir is MarketDirection.BEARISH
                        and standing_choch_ref is not None
                    ):
                        bear_choch_rearm = Pivot(
                            price=standing_choch_ref.price,
                            timestamp=break_candle.timestamp,
                        )
                        bear_choch_rearm_weak = (
                            bear_choch_fail_ref_weak
                            if bear_choch_fail_ref is not None
                            else False
                        )
                        bear_choch_rearm_arm_index = current_index
                        # A fresh failure re-arms at full (pre-confirmation)
                        # rank; demotion re-applies at the next confirmation.
                        bear_choch_rearm_demoted = False
                    bear_choch_from_rearm = False
                    ...
                    and choch_high_scan_start <= current_index
                    and confirms_break(
                    ...
                    trend_confirmed = False
                    # Re-activation bookkeeping (`choch_failed_rearm`): a
                    # re-fired CHoCH must not re-arm on its own failure
                    # (one-shot chain); any CHoCH emission drops both
                    # memories (the fresh cycle owns the narrative).
                    bull_choch_from_rearm = (
                        rearm_high_ref is not None and choch_high_ref is rearm_high_ref
                    )
                    bear_choch_from_rearm = False
                    bull_choch_rearm = None
                    ...
                    # origin. See `choch_weak_ref_fail_at_broken_level`. Under
                    # `choch_pending_fail_at_broken_level` a *structural*
                    # reference arms too (at the pending-fail persistence),
                    # so an unconfirmed CHoCH cannot outlive a sustained
                    # reclaim of the very level it broke.
                    bull_choch_fail_ref = None
                    bull_choch_fail_ref_weak = False
                    if choch_high_weak_ref:
                ...
                if pending_bos is not None and pending_bos.direction is MarketDirection.BULLISH:
                    ...
                    if (
                        ...
                        if self._pullback_quality_ok(candles[current_index], high_pivot=False):
                            ...
                            # An emitted in-trend BOS confirms the structure:
                            # the confirmed-trend barrier now governs any
                            # bearish reversal CHoCH. A confirmed bullish trend
                            # also retires the bearish re-arm memory (see
                            # `choch_failed_rearm`) -- or, under
                            # `choch_failed_rearm_persistent`, demotes it
                            # below the fallback to catch a sweep-shaped top
                            # without shadowing nearer references.
                            trend_confirmed = True
                            if self._choch_failed_rearm_persistent:
                                bear_choch_rearm_demoted = True
                            else:
                                bear_choch_rearm = None
                        elif (
                            self._stage_wick_rejected_bos
                ...
                if (
                    ...
                    fallback_active_low = None
                # Failed-CHoCH re-activation reference (mirror of the high
                # side; see `choch_failed_rearm`).
                rearm_low_ref = bear_choch_rearm if self._choch_failed_rearm else None
                choch_low_ref = (
                    validated_choch_low
                    or pending_leg_origin_low
                    or choch_origin_low
                )
                if choch_low_ref is None:
                    if (
                        bear_choch_rearm_demoted
                        and rearm_low_ref is not None
                        and fallback_active_low is not None
                    ):
                        # Mirror of the high side: a demoted re-arm and a live
                        # fallback are arbitrated by whichever level price
                        # crosses first in the break direction (a bearish
                        # CHoCH breaks downward, so the higher level). See
                        # `choch_failed_rearm_persistent`.
                        choch_low_ref = (
                            rearm_low_ref
                            if rearm_low_ref.price >= fallback_active_low.price
                            else fallback_active_low
                        )
                    else:
                        choch_low_ref = rearm_low_ref or fallback_active_low
                ...
                if rearm_low_ref is not None and choch_low_ref is rearm_low_ref:
                    choch_low_weak_ref = bear_choch_rearm_weak
                ...
                # Mirror of the bearish case: a structural-level failure (the
                ...
                if (
                    ...
                    and not bull_choch_fail_ref_weak
                    and self._choch_pending_fail_persistence_candles is not None
                ):
                ...
                if (
                    ...
                    # CHoCH rally launched from (or, for a level-armed CHoCH,
                    ...
                    # long-dead level.
                    bear_leg_low = price
                    # Mirror of the bearish case: only a *weak*-level failure
                    # re-seeds at the reclaimed level; a structural-level
                    # failure restores the pre-CHoCH stash (the interrupted
                    # cycle was alive).
                    weak_level_failure = (
                        bull_fail_pivot is not bull_choch_origin
                    ...
                    pre_choch_bear_bos_extreme = None
                    pre_choch_bull_bos_extreme = None
                    # Failed-CHoCH re-activation (mirror of the bearish
                    # failure; see `choch_failed_rearm`): arm the failed
                    # bullish CHoCH's broken level so a later sustained break
                    # back above it re-fires the CHoCH. Level price + the
                    # failure's timestamp, so the re-fired line starts at the
                    # failure (see the bearish mirror).
                    if (
                        self._choch_failed_rearm
                        and (
                            not bull_choch_from_rearm
                            or self._choch_failed_rearm_persistent
                        )
                        and standing_choch_dir is MarketDirection.BULLISH
                        and standing_choch_ref is not None
                    ):
                        bull_choch_rearm = Pivot(
                            price=standing_choch_ref.price,
                            timestamp=break_candle.timestamp,
                        )
                        bull_choch_rearm_weak = (
                            bull_choch_fail_ref_weak
                            if bull_choch_fail_ref is not None
                            else False
                        )
                        bull_choch_rearm_arm_index = current_index
                        # Fresh arm at full rank (see the bearish mirror).
                        bull_choch_rearm_demoted = False
                    bull_choch_from_rearm = False
                    # One-shot: a failed-CHoCH flip does NOT arm the opposite
                    # origin / blind-spot fallback, so failures cannot ping-pong.
                    ...
                    and choch_low_scan_start <= current_index
                    and confirms_break(
                    ...
                    # confirms it (see `choch_confirmed_trend_persistence_candles`).
                    trend_confirmed = False
                    # Re-activation bookkeeping (mirror of the bullish case;
                    # see `choch_failed_rearm`).
                    bear_choch_from_rearm = (
                        rearm_low_ref is not None and choch_low_ref is rearm_low_ref
                    )
                    bull_choch_from_rearm = False
                    bear_choch_rearm = None
                    bull_choch_rearm = None
                    ...
                    # `choch_weak_ref_fail_at_broken_level`.
                    # Mirror of the bullish arming: a structural reference arms
                    # too under `choch_pending_fail_at_broken_level`, so an
                    # unconfirmed CHoCH cannot outlive a sustained reclaim of
                    # the level it broke.
                    bear_choch_fail_ref = None
                    bear_choch_fail_ref_weak = False
                    if choch_low_weak_ref:
        ...
        # Live-edge CHOCH_FAILED (only under `choch_fail_live_edge`). The
        # in-loop failure checks are pivot-gated, so on a relentless one-way
        # move no pivot forms and a long-since-invalidated CHoCH keeps its
        # wrong trend standing at the live edge (the MUUSDT H4 case: -19%
        # below the fail level, trend still bullish). The failure condition is
        # complete -- only the pivot messenger is late -- so run the same check
        # (same reference arbitration, persistence class, and
        # displacement-success retirement) once more over the final state and
        # emit the real failure at the sustained-break candle. The full
        # in-loop failure bookkeeping is deliberately skipped: it only feeds
        # subsequent pivots, of which there are none, and the next run that
        # *does* see a pivot re-derives the identical event through the
        # in-loop path.
        ...
        if self._choch_fail_live_edge and candles:
            ...
            if trend is MarketDirection.BULLISH:
                ...
                if (
                    ...
                    and le_fail_pivot is bull_choch_fail_ref
                    and not bull_choch_fail_ref_weak
                    and self._choch_pending_fail_persistence_candles is not None
                ):
                ...
                if (
                    ...
                    le_break_i = find_sustained_break_index(
                        candles,
                        le_scan_start,
                        last_index,
                        le_fail_level,
                    ...
                    emit(
                        candles[le_break_i].timestamp,
                        StructureEvent.CHOCH_FAILED,
                        MarketDirection.BULLISH,
                        min(c.low for c in candles[le_break_i:]),
                        le_fail_pivot.price,
                        reference_timestamp=le_fail_pivot.timestamp,
                    )
                    trend = MarketDirection.BEARISH
                    trend_confirmed = False
                    live_edge_fail = True
                ...
                if (
                    le_fail_pivot is not None
                    and le_fail_pivot is bear_choch_fail_ref
                    and not bear_choch_fail_ref_weak
                    and self._choch_pending_fail_persistence_candles is not None
                ):
                    le_fail_persistence = self._choch_pending_fail_persistence_candles
                ...
                if (
                    ...
                    emit(
                        ...
                        StructureEvent.CHOCH_FAILED,
                        MarketDirection.BEARISH,
                        max(c.high for c in candles[le_break_i:]),
                        le_fail_pivot.price,
                        reference_timestamp=le_fail_pivot.timestamp,
                    )
                    trend = MarketDirection.BULLISH
        ...
        if (
            ...
            # state below reflects the pre-flip trend -- skip this run.
        ...
        if (
            self._emit_provisional_choch
            and trend is not MarketDirection.NEUTRAL
            # Same live-edge-failure suppression as the provisional BOS above.
            and not live_edge_fail
        ):
            tail = candles[last_advance_index + 1 :] if last_advance_index >= 0 else []
        ...
        # A live-edge failure supersedes the fizzle marker: the real failure
        # already ended the standing CHoCH's line (and the flipped trend no
        # longer matches `standing_choch_dir` anyway).
        if self._choch_fizzle_reclaim_candles is not None and not live_edge_fail:
            need = self._persistence_candles
    ...
    def _fail_break_level(
        self,
        level: float,
        mean_tr_pct: float | None,
        *,
        bullish_reclaim: bool,
    ) -> float:
        """The level a CHoCH-failure reclaim must actually close beyond.

        The bare level is the CHoCH's own reference, and price retests it
        constantly: two closes a hair past it (0.37 mean-TR on the BTC 15m
        2026-07-25 case) killed a reversal that then ran another 1.9%. Under
        `choch_fail_level_buffer_atr` the reclaim is measured against the level
        offset by a volatility-scaled noise band, so an ordinary retest no
        longer invalidates the CHoCH while a genuine loss of the level still
        does. Returns the level unchanged when the feature is off or the series
        is too short to measure a range.
        """
        if self._choch_fail_level_buffer_atr is None or mean_tr_pct is None:
            return level
        frac = self._choch_fail_level_buffer_atr * mean_tr_pct
        return level * (1.0 + frac) if bullish_reclaim else level * (1.0 - frac)
